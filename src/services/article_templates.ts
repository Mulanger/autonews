...
function buildTradeTemplate(event: ArticleEvent): ArticleDraft {
  ...
  const titles = [
    `${subject} ${action} ${facts.outcome} with ${amount} on Polymarket`,
    `A ${amount} whale trade just hit ${facts.marketTitle}`,
    `${subject}'s ${amount} move puts ${facts.marketTitle} in focus`,
  ...
  return {
    title,
    dek: `${subject} placed a ${formatUsd(facts.amountUsd)} ${facts.side} trade on ${facts.outcome} at ${price}, according to Polywhale-tracked activity.`,
    body: [
      `Polywhale detected a ${formatUsd(facts.amountUsd)} whale trade on ${facts.marketTitle}. ${name} ${facts.side === 'BUY' ? 'bought' : 'sold'} ${facts.outcome} shares at ${price}.`,
...
function buildLossTemplate(event: ArticleEvent): ArticleDraft {
  ...
  const titles = [
    `${subject} just lost ${loss} on ${facts.marketTitle}`,
  ...
  return {
    title,
    dek: `${subject}'s ${formatUsd(facts.amountUsd)} ${facts.outcome} position resolved against the wallet, leaving an estimated ${formatUsd(facts.lossUsd ?? Math.abs(facts.pnlUsd ?? facts.amountUsd))} loss.`,
    body: [
      `A whale position tracked by Polywhale has resolved as a loss. ${name} held ${facts.outcome} on ${facts.marketTitle}, while the market resolved to ${facts.winningOutcome ?? 'the other side'}.`,
